...
public class Sword : Weapon
{
    ...
    // Range of slash attack, automatically set to collider length of slashObj
    private float _slashRange;
    ...
    public override void ReleaseSecondaryFire()
    {
        _isSlashHeld = false;

        if (currentAction == SwordActions.ChargeSlash)
        {
            SlashRelease();
        }
    }
    ...
    public override void AbortCurrentAction()
    {
        ...
        {
            case SwordActions.ChargeSlash:
                CancelInvoke(nameof(ChargeSlash));
    ...
    {
        currentAction = SwordActions.ChargeSlash;

        _timeSlashBegan = Time.time;

        Invoke(nameof(ChargeSlash), _slashMinHoldTime);
    }
